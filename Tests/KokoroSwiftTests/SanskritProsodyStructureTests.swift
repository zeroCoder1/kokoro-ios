...
// MARK: - The traditional pace

// Calibrated against 668 verses of a recorded Gītā pāṭha, not chosen by ear.
// The reciter takes a median 15.42 s of speech per anuṣṭubh verse — 482 ms per
// syllable, against our 283 ms at speed 0.80.

@Test func traditionalIsTheSlowestDelivery() {
  for other in [SanskritDelivery.learning, .recitation, .fast, .unshaped] {
    #expect(SanskritDelivery.traditional.speed < other.speed,
            "traditional is not slower than \(other.speed)")
  }
  // Not a round number chosen for feel: 0.30 is where the swept voiced time
  // per syllable lands on the reciter's 477 ms — 473, 474 and 478 across the
  // three validation verses. 0.36 gives 403 and 0.25 gives 564.
  #expect(SanskritDelivery.traditional.speed == 0.30)
}

/// The half-verse pause is far *shorter* here, and the reason inverts.
///
/// Kokoro's own gap at the daṇḍa grows as it slows — 390–460 ms at 0.80 but
/// 550–1150 ms at 0.30 — so at this pace the model over-pauses and the split
/// path is what reins it in. `padaPause` is divided by speed at render time,
/// so what must match the reciter's measured 410 ms is the quotient.
@Test func traditionalPausesLikeTheReciter() {
  #expect(SanskritDelivery.traditional.prosody.padaPause
          < SanskritDelivery.recitation.prosody.padaPause)

  func gap(_ delivery: SanskritDelivery) -> Double {
    delivery.prosody.padaPause / Double(delivery.speed)
  }
  // The reciter: 410 ms, median over the 292 anuṣṭubh verses that take a break.
  #expect(abs(gap(.traditional) - 0.410) < 0.05,
          "traditional's half-verse gap is \(gap(.traditional))s, not the measured 0.41s")
  // Closer than the deliveries that actually hold a break. (`fast` lands on
  // 0.40 too, at speed 1.0 — a coincidence, not a calibration.)
  for other in [SanskritDelivery.learning, .recitation] {
    #expect(abs(gap(.traditional) - 0.410) < abs(gap(other) - 0.410))
  }

  // versePause is deliberately uncalibrated — the reference recordings are
  // trimmed at the end — and is simply twice the half-verse break.
  #expect(SanskritDelivery.traditional.prosody.versePause
          == SanskritDelivery.traditional.prosody.padaPause * 2)
}

/// A pace is not a pronunciation. Nothing about the slower delivery may reach
/// the phonemes, the tokens or the segment split.
@Test func traditionalChangesTimingOnly() {
  let verses = [
    "धर्मक्षेत्रे कुरुक्षेत्रे समवेता युयुत्सवः ।\nमामकाः पाण्डवाश्चैव किमकुर्वत सञ्जय ॥",
    "कर्मण्येवाधिकारस्ते मा फलेषु कदाचन ।\nमा कर्मफलहेतुर्भूर्मा ते सङ्गोऽस्त्वकर्मणि ॥",
  ]
  for verse in verses {
    let reference = SanskritProsody.segments(
      for: verse, configuration: SanskritDelivery.recitation.prosody
    )
    let traditional = SanskritProsody.segments(
      for: verse, configuration: SanskritDelivery.traditional.prosody
    )
    #expect(traditional.map(\.phonemes) == reference.map(\.phonemes),
            "traditional changed the phonemes")
    #expect(traditional.map(\.boundary) == reference.map(\.boundary),
            "traditional changed where the verse splits")
    // Two segments either way: one half-verse break, which is exactly what the
    // reciter takes. Only the silence between them differs.
    #expect(traditional.count == 2)
    #expect(traditional[0].pauseAfter < reference[0].pauseAfter)
  }
  // The duration intent is the shipped one, not an experimental profile.
  #expect(SanskritDelivery.traditional.intent == .closureRepairs)
}
